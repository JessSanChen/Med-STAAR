// src/partials/FacilityCoverageSheet.jsx
...
function parseCSV(text) {
  ...
  while (i < text.length) {
    ...
    if (inQuotes) {
      ...
      else if (ch === '\r') { if (text[i + 1] === '\n') i++; row.push(cell); rows.push(row); row = []; cell = ''; i++; }
...
export default function FacilityCoverageSheet() {
  ...
  useEffect(() => {
    const url = `${window.location.origin}/facility_coverage.csv`;
    (async () => {
      try {
        ...
        if (!res.ok || ctype.includes('text/html') || text.startsWith('<!DOCTYPE')) {
          throw new Error('CSV not found at /facility_coverage.csv (got HTML). Put the file in /public.');
        }
